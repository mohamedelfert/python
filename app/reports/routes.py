...
from app.reports.leads.leadsOverallReport import leadsOverallReport
...
# meetings overall report
@bp.route('/meetings-overall', methods=["POST"])
@jwt_required()
def meetings_overall():
    return jsonify(meetingsOverallReport())
...
# leads overall report
@bp.route('/leads-overall', methods=["POST"])
@jwt_required()
def leads_overall():
    return jsonify(leadsOverallReport())
